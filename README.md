...
- **効率**: 実際にその効果へ到達するマナ、速度、追加コスト
- **柔軟性**: モード、複数役割、腐りにくさ
- **信頼性**: 対象範囲、条件の少なさ、状況依存度
- **デッキ適合**: 色、Rule/Knowledge接続、既存構造との相性
- **Evidence**: User FeedbackとAdoption Evidence。採用率をそのまま強さにはしない

### 除去／妨害の品質差
- 汎用的な確定除去、追放、クリーチャー＋プレインズウォーカー対応、インスタント速度を高く評価
- 固定点数火力は点数に応じて信頼性を区別
- 飛行限定、攻撃中限定、サイズ制限、条件付き、自己生け贄などは明示的に減点
- 小型一掃と汎用除去を同一品質として扱わない
- Candidate Qualityが低いカードは、役割タグが一致していてもOptimizer候補から外す

### OUT保護
Card Valueが非常に高い多機能カードを、低～中品質のIN候補と安易に交換しない保護を追加します。User-confirmed Coreは従来どおり最優先でOUTから除外します。

### 色追加候補
現在色の外でも、1色・単一色シンボル程度で、Candidate Qualityが十分高く最優先課題へ直接答えるカードは「色追加候補」として検討可能にします。環境Evidenceが強い場合も補助します。複数色追加や強い色拘束は引き続き強く抑制します。

### 重要な制限
Candidate Qualityは勝率予測でも絶対的なカードパワーランキングでもありません。プレイ実績、メタゲーム、対戦相手、実際のゲーム中の選択を完全には再現できないため、User EvidenceとAdoption Evidenceを今後も継続的に育てる前提の評価です。
