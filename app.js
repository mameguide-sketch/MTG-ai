...
function cardHTML(x){return `<article class="result"><button class="imageButton" data-detail="${esc(x.card.name)}" aria-label="${esc(x.card.name)}の詳細">${img(x.card)?`<img loading="lazy" src="${img(x.card)}" alt="${esc(x.card.name)}">`:''}</button><div><h3><button class="textButton" data-detail="${esc(x.card.name)}">${esc(x.card.name)}</button></h3><div class="meta">${esc(type(x.card))} ・ MV ${x.card.cmc||0} ・ ${colors(x.f.colors)}</div><div class="tags">${x.why.map(w=>`<span class="tag good">${esc(w)}</span>`).join('')}${x.f.roles.slice(0,4).map(r=>`<span class="tag">${labels[r]||r}</span>`).join('')}</div><div class="oracle">${esc(oracle(x.card))}</div><div class="inlineActions"><button class="smallBtn primary" data-deckadd="${esc(x.card.name)}">デッキへ追加</button><button class="smallBtn" data-detail="${esc(x.card.name)}">詳細を見る</button></div></div><div><div class="score">${x.score}</div><div class="tiny">適合点</div></div></article>`}
...
function populateCardNames(){
  const dl=$('cardNames');
  if(!dl||!pool.length)return;
  dl.innerHTML=pool.slice().sort((a,b)=>a.name.localeCompare(b.name)).map(c=>`<option value="${esc(c.name)}"></option>`).join('');
}
function cardSearchText(c){return `${c.name} ${type(c)} ${text(c)} ${(c.keywords||[]).join(' ')}`.toLowerCase()}
...
function findPoolCard(name){return pool.find(c=>c.name===name)||pool.find(c=>c.name.toLowerCase()===String(name).toLowerCase())}
function showCardDetail(name){
  const c=findPoolCard(name);if(!c)return;
  const f=features(c),legal=c.legalities?.standard==='legal';
  $('dialogContent').innerHTML=`<div class="dialogGrid"><div>${img(c)?`<img class="dialogImage" src="${img(c)}" alt="${esc(c.name)}">`:''}</div><div><div class="dialogEyebrow">${legal?'スタンダード使用可':'スタンダード対象外'}</div><h2>${esc(c.name)}</h2><div class="manaCost">${esc(manaCost(c))}</div><div class="meta">${esc(type(c))} ・ MV ${c.cmc||0} ・ ${colors(f.colors)}</div><div class="tags dialogTags">${f.roles.length?f.roles.map(r=>`<span class="tag">${labels[r]||r}</span>`).join(''):'<span class="tag">役割未分類</span>'}</div><div class="dialogOracle">${esc(oracle(c))}</div>${c.power!=null?`<div class="statLine">P/T：${esc(c.power)} / ${esc(c.toughness)}</div>`:''}<div class="dialogActions"><button class="btn" data-deckadd="${esc(c.name)}">1枚デッキへ追加</button><button class="btn secondary" data-synergy="${esc(c.name)}">このカードの相性を見る</button></div><div class="tiny">収録：${esc(c.set_name||'—')} ${c.released_at?`・ ${esc(c.released_at)}`:''}</div></div></div>`;
