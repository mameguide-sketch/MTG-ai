...
## v0.7.3f 即時学習チェック
- [ ] GOODクリック直後に学習通知が表示され、操作不能にならない
- [ ] BADクリック直後に学習通知が表示され、該当する表示中候補が即時除外される
- [ ] Core指定／解除で長時間UIが固まらない
- [ ] BAD後、次回「再分析」でBad学習が全候補に反映される
- [ ] GOOD後、次回「再分析」でGood学習が候補順位へ反映される
- [ ] 「最適化案を再計算」は明示的なフル再計算として動作する
