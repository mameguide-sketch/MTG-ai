# v0.7.0 Deck Optimization Engine α 確認項目
- [ ] 画面上部が v0.7.0
- [ ] デッキ分析に「デッキ最適化」パネルが表示される
- [ ] デッキ分析後に現在のDeck Foundation scoreが表示される
- [ ] 「最優先課題」が表示される
- [ ] 不足役割・不足/孤立・土地枚数・マナカーブなどから優先課題が表示される
- [ ] 「自動保護する構成要素」が表示される
- [ ] 検証済みシナジーの構成カードが自動OUT候補から除外される
- [ ] 1枚減らすと効果接続や不足が悪化するカードは保護/注意として表示される
- [ ] 最適化案に OUT → IN が表示される
- [ ] 最適化案に変更前 → 変更後のデッキ全体評価と改善値が表示される
- [ ] 最大変更枚数を2枚以上にすると、条件が合えば複数交換をまとめた案も表示される
- [ ] 「この最適化案を適用」でメインデッキだけが変更される
- [ ] 最適化案を適用してもサイドボードは変化しない
- [ ] Undoで最適化前のメイン / サイドへ戻せる
- [ ] Swap Planner側でも検証済みシナジー中核を安易に削減しない
- [ ] 全体評価が悪化する最適化案は表示されない
- [ ] 通常土地は引き続き「マナ加速」に含まれない
- [ ] Main / Sideboard分離が維持されている
- [ ] ルール検証は Rule Engine β-9 の既存機能が維持されている
- [ ] 月影セットの理論最速2Tが維持されている
